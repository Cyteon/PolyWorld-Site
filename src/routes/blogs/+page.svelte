...
<svelte:head>
    <title>Blogs</title>
    <meta name="og:url" content="https://polyworld.xyz/blogs" />
    <meta name="og:title" content="Polyworld | Blog" />
</svelte:head>
...
<div class="min-h-screen w-full flex px-1">
    <div class="md:w-1/2 mx-auto mt-2">
        ...
        <div class="md:grid grid-cols-2 gap-4 mt-4">
            {#each posts as post}
                <a href={`/blogs/${post.slug}`} class="flex flex-col border border-neutral-700 p-2 rounded-md transition-all duration-300 hover:scale-105">
